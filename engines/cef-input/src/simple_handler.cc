...
SimpleHandler::SimpleHandler(const std::function<void(const void*, int, int)>& func) : func_(func) {
  DCHECK(!g_instance);
  g_instance = this;
}
...
void SimpleHandler::OnPaint(CefRefPtr<CefBrowser> browser, PaintElementType type, const RectList& dirtyRects, const void* buffer, int w, int h) {
  func_(buffer, w, h);
}
